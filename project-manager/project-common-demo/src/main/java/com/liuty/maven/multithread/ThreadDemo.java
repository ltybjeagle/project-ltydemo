 ...
 * 并发：同一时间段内多个任务同时执行
 * 并行：单位时间内多个任务同时执行
 * 线程：JAVA线程与操作系统线程一一对应
 * JAVA内存模型：
 *      主内存、工作内存（CPU寄存器、一级缓存、二级缓存）、数据一致性
 *      伪共享：
 *          1、CPU缓存是以缓存行的形式缓存数据（缓存行可以存多个变量），
 *          缓存与主内存交换数据也是以缓存行为单位交换数据
 *          2、缓存行的大小一般为2的幂次数字节
 *          3、多线程环境，伪共享会影响性能（线程更新缓存，缓存一致性会将缓存行整体失效）
 *          4、JDK8提供@Contended注解，此注解可以自动填充缓存行数据，以此保证缓存行只保存一个变量，
 *          使用此注解，需要JVM参数：-XX:-RestrictContended
 */
