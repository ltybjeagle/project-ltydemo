...
public class DynamicDataSourceRegister implements EnvironmentAware, ImportBeanDefinitionRegistrar {
    ...
    private boolean isSupportJta(){
        String jta = evn.getProperty("spring.datasource.jta","true");
        return ("true".equals(jta));
    }
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata,
        ...
        if (binder.bind("spring.datasource.multi", Map.class).isBound()) {
            // 获取其他数据源配置
            ...
            for (Map map : configs) {
                ...
                // 获取数据源的key，以便通过该key可以定位到数据源
                dataSources.put(key, consumerDatasource);
        ...
        define.setBeanClass(DynamicDataSourceRouter.class);
        ...
        BeanDefinitionRegistry registry = ((BeanDefinitionRegistry)
                ((DefaultListableBeanFactory) beanDefinitionRegistry).getParentBeanFactory());
        registry.registerBeanDefinition("datasource", define);
        logger.info("注册数据源成功，一共注册{}个数据源", dataSources.keySet().size() + 1);
    }
