package com.sunny.maven.rpc.reflect.asm.proxy;

import com.sunny.maven.rpc.reflect.asm.visitor.TargetClassVisitor;
import com.sunny.maven.rpc.reflect.asm.visitor.TargetClassVisitor.MethodBean;
 ...
 * @author SUNNY
 * @description: 反射的对象代理
 * @create: 2023-02-01 09:59
 */
public class ReflectProxy {
    public static final int ASM_VERSION = Opcodes.ASM5;
    public static final int ASM_JDK_VERSION = Opcodes.V11;
    /**
     * 动态生成代理类的前缀
     */
     ...
     * @return
     */
    public static Object newProxyInstance(ClassLoader classLoader, InvocationHandler invocationHandler,
                                          Class<?> targetClass, Constructor<?> targetConstructor,
                                          Object ...targetParam) {
        ...
        try {
            ...
            // 创建类
            newClass(classWriter, newClassInnerName, targetClassInnerName);
            // 添加 InvocationHandler 字段
            addField(classWriter);
            // 添加 InvocationHandler 的setter
            addSetterMethod(classWriter, newClassInnerName);
            // 添加构造器，直接调用 super
            List<MethodBean> constructors = targetClassVisitor.getConstructors();
            addConstructor(classWriter, constructors, targetClassInnerName);
            // 添加调用 InvocationHandler 的方法
            addInvokeMethod(classWriter, newClassInnerName);
            // 添加继承的public方法和目标类的protected、default方法
            List<MethodBean> methods = targetClassVisitor.getMethods();
            List<MethodBean> declareMethods = targetClassVisitor.getDeclareMethods();
            Map<Integer, Integer> methodsMap = new HashMap<>();
            Map<Integer, Integer> declareMethodsMap = new HashMap<>();
            int methodNameIndex = 0;
            methodNameIndex = addMethod(classWriter, newClassInnerName, targetClass.getMethods(), methods,
                    true, methodNameIndex, methodsMap);
            addMethod(classWriter, newClassInnerName, targetClass.getDeclaredMethods(), declareMethods,
                    false, methodNameIndex, declareMethodsMap);
            // 添加静态代码块的初始化
            addStaticInitBlock(classWriter, targetClassName, newClassInnerName, methodsMap, declareMethodsMap);
            // 生成二进制数据
     ...
     * 创建类
     */
    private static void newClass(ClassWriter classWriter, String newClassName, String targetClassName) {
    ...
    private static void addField(ClassWriter classWriter) throws Exception {
        FieldVisitor fieldVisitor = classWriter.visitField(Opcodes.ACC_PRIVATE, FIELD_INVOCATIONHANDLER,
     ...
     */
    private static void addSetterMethod(ClassWriter classWriter, String owner) throws Exception {
        String methodDesc = "(" + Type.getDescriptor(InvocationHandler.class) + ")V";
        MethodVisitor methodVisitor = classWriter.visitMethod(Opcodes.ACC_PUBLIC, METHOD_SETTER, methodDesc,
                null, null);
     ...
     * 添加构造器
     */
    private static void addConstructor(ClassWriter classWriter, List<MethodBean> constructors,
        ...
        for (MethodBean constructor : constructors) {
            ...
            // 调用super() 构造器
            methodVisitor.visitMethodInsn(Opcodes.INVOKESPECIAL, targetClassInnerName, "<init>",
                    constructor.methodDesc, false);
            methodVisitor.visitInsn(Opcodes.RETURN);
    ...
    private static void addInvokeMethod(ClassWriter classWriter, String owner) {
        MethodVisitor methodVisitor = classWriter.visitMethod(Opcodes.ACC_PRIVATE | Opcodes.ACC_VARARGS,
        ...
        methodVisitor.visitMethodInsn(Opcodes.INVOKEINTERFACE, handlerName, handlerMethodName, handlerDesc,
                true);
        // 正常返回
     ...
     * 添加继承的方法或目标类本身的方法
     */
    private static int addMethod(ClassWriter classWriter, String newClassInnerName, Method[] methods,
                                 List<MethodBean> methodBeans, boolean isPublic, int methodNameIndex,
        ...
        for (int i = 0; i < methodBeans.size(); i++) {
            ...
            fieldVisitor.visitEnd();
            // 添加方法的调用
            addMethod(classWriter, newClassInnerName, methodBean, access, methodNameIndex);
            methodNameIndex++;
        }
     ...
     * 实现方法的调用
     */
    private static void addMethod(ClassWriter classWriter, String newClassInnerName, MethodBean methodBean,
                                  int access, int methodNameIndex) {
        MethodVisitor methodVisitor = classWriter.visitMethod(access, methodBean.methodName, methodBean.methodDesc,
                null, null);
        methodVisitor.visitCode();
        methodVisitor.visitVarInsn(Opcodes.ALOAD, 0);
        // 区分静态或者是非静态方法调用
        ...
        // 获取新建的方法字段
        methodVisitor.visitFieldInsn(Opcodes.GETSTATIC, newClassInnerName, METHOD_FIELD_PREFIX + methodNameIndex,
        ...
        methodVisitor.visitMethodInsn(Opcodes.INVOKESPECIAL, newClassInnerName, METHOD_INVOKE, METHOD_INVOKE_DESC,
                false);
        // 处理返回情况，基本类型需要拆箱
     ...
     * 添加静态初始代码块
     */
    private static void addStaticInitBlock(ClassWriter classWriter, String targetClassName, String newClassInnerName,
                                  Map<Integer, Integer> methodMap, Map<Integer, Integer> declareMethodsMap)
            throws Exception {
        String exceptionClassName = Type.getInternalName(ClassNotFoundException.class);
        MethodVisitor methodVisitor = classWriter.visitMethod(Opcodes.ACC_STATIC, "<clinit>", "()V",
                null, null);
        methodVisitor.visitCode();
        // 异常处理
        Label label0 = new Label();
        ...
        methodVisitor.visitLabel(label0);
        // 给继承的方法添加对应的字段初始化
        for (Map.Entry<Integer, Integer> entry : methodMap.entrySet()) {
            ...
            methodVisitor.visitFieldInsn(Opcodes.PUTSTATIC, newClassInnerName, METHOD_FIELD_PREFIX + key,
                    Type.getDescriptor(Method.class));
        }
        // 给目标类本身的方法添加对应的字段初始化
        for (Map.Entry<Integer, Integer> entry : declareMethodsMap.entrySet()) {
            Integer key = entry.getKey();
            Integer value = entry.getValue();
            methodVisitor.visitLdcInsn(targetClassName);
            methodVisitor.visitMethodInsn(Opcodes.INVOKESTATIC, Type.getInternalName(Class.class),
        ...
        methodVisitor.visitVarInsn(Opcodes.ALOAD, 0);
        methodVisitor.visitMethodInsn(Opcodes.INVOKEVIRTUAL, exceptionClassName, "printStackTrace",
                "()V", false);
    ...
    private static boolean equalsMethod(Method method, MethodBean methodBean) {
        ...
        try {
            ...
            Type[] argumentType1 = Type.getArgumentTypes(method);
            Type[] argumentType2 = Type.getArgumentTypes(methodBean.methodDesc);
            if (argumentType1.length != argumentType2.length) {
                return false;
            }
            for (int i = 0; i < argumentType1.length; i++) {
                if (!argumentType1[i].equals(argumentType2[i])) {
                    return false;
                }
            }
            return true;
