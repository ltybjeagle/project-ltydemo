package com.sunny.maven.microservice.gateway.config;
 ...
 * @author SUNNY
 * @description: 网关配置类
 * @create: 2023-04-01 15:47
 */
@Configuration
@ConditionalOnProperty(name = "gateway.router.config.enable", havingValue = "true")
public class GateWayConfig {
    ...
    private void initGatewayRules() {
         ...
         * 以达到启动项目即可实现访问链接的限流效果，而无需登录Setinel管理界面手动配置限流规则，可以将
         * resource参数设置为${spring.cloud.gateway.discovery.locator.route-id-prefix}目标微服务的名称
         *
         * 当然，如果不按照上述配置，也可以在项目启动后，通过服务网关访问目标微服务链接后，
         * 在Sentinel管理界面的请求链路中找到对应的API名称所代表的请求链路，然后手动配置限流规则。
         **/
        // 用户微服务网关
        rules.add(this.getGatewayFlowRule(this.getResource("microservice-user")));
        // 商品微服务网关
        rules.add(this.getGatewayFlowRule(this.getResource("microservice-product")));
        // 订单微服务网关
        rules.add(this.getGatewayFlowRule(this.getResource("microservice-order")));
    ...
    private void initCustomizedApis() {
        ...
        ApiDefinition api1 = new ApiDefinition("user-api1").
                setPredicateItems(new HashSet<ApiPredicateItem>(){{
                    // 以/microservice-user/user/api1 开头的请求
                    add(new ApiPathPredicateItem().setPattern("/microservice-user/user/api1/**").
                            setMatchStrategy(SentinelGatewayConstants.URL_MATCH_STRATEGY_PREFIX));
                }});
        ApiDefinition api2 = new ApiDefinition("user-api2").
                setPredicateItems(new HashSet<ApiPredicateItem>(){{
                    // 以/microservice-user/user/api2/demo1 完成的url路径匹配
                    add(new ApiPathPredicateItem().setPattern("/microservice-user/user/api2/demo1"));
                }});
